...
/**
 * Redic继承自原生Jedis.提供了与Jedis完全一致的API，并且覆盖了原有Jedis的API实现，
 * 实现了基于关键字哈希的路由规则。
 * <p>
 * 维护一个RedicNode对象的集合，每个元素就是一个分片节点，每次请求都会根据哈希规则落到其中某个节点进行处理，
 * 这里默认使用关键字哈希的路由规则。
 * <p>
 * 路由规则封装在接口ShardingStrategy和实现类HashShardingStrategy中。
 */
public class Redic extends Jedis {
    ...
    /**
     * 先基于路由规则，获取相应key对应的Jedis读写资源，
     * 然后委托给此Jedis资源进行相应的API操作。
     */
     ...
     *
     * @param key   the key of the redis variable.
     * @param value the value to minus off.
